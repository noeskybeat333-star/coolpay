...
Импорт заказов WB FBS завершён и закоммичен. Цены и остатки 15.08.2026
отделены от импорта каталога, `MarketplaceCooldown` починен и покрыт
тестами. Дальше по приоритету:

0. **Разобраться с типом токена WB.** `X-Ratelimit-Limit: 1` против
   документированных 10–20 держится стабильно, и все упирания в лимит
   идут отсюда. Проверить в кабинете, не помечен ли токен как тестовый;
   если да — выпустить боевой. Пока это не сделано, любые выводы о
   производительности импорта преждевременны.
